...
const SearchableFilterInventoryItem = ({
  ...
  return (
    <div className={clsx("w-full border-b")}>
      ...
      >
        <div className="mb-2 flex w-full flex-col gap-y-xsmall pt-2">
          <InputField
            value={query}
            className="pr-1"
            prefix={
              selectedItems.size === 0 ? null : (
                <div
                  onClick={reset}
                  className="mr-xsmall flex cursor-pointer items-center gap-x-2xsmall rounded-rounded border border-grey-20 bg-grey-10 py-0.5 pr-1 pl-2 text-grey-40"
                >
...
const InventoryItemItem = ({
  ...
  <div
    key={key}
    onClick={onClick}
    className="flex items-center rounded-rounded py-1.5 px-2 hover:bg-grey-10"
  >
...
const CreatedByItem = ({
  ...
  return (
    <div
      key={key}
      onClick={onClick}
      className="inter-small-regular flex items-center rounded-rounded py-1.5 px-2 hover:bg-grey-10"
    >
...
const TextFilterItem = ({
  ...
  return (
    <div className={clsx("w-full border-b")}>
      ...
      >
        <div className="flex w-full flex-col gap-y-xsmall py-2">
          <PopoverSelect
            options={options}
            value={filterType}
...
const FilterDatePicker = ({
  ...
  <RadixPopover.Root>
    <RadixPopover.Trigger className="w-full">
      <div className="inter-small-regular flex w-full items-center rounded-rounded border bg-grey-5 py-1.5 pl-3 pr-2 text-left">
        <span className="mr-1.5 text-grey-40">
          <CalendarIcon size={20} />
        </span>
        {date ? (
          <p className="mt-0.5">{moment(date).format("MMM DD, YYYY")}</p>
        ) : (
          <p className="mt-0.5 text-grey-40">Pick a date</p>
        )}
      </div>
    ...
    <RadixPopover.Content
      side={side}
      sideOffset={8}
      className="rounded-rounded border bg-grey-0 p-1"
    >
...
const PopoverSelect = ({
  ...
  return (
    <RadixPopover.Root>
      <RadixPopover.Trigger className="w-full">
        <div className="w-full rounded-rounded border bg-grey-5 py-1.5 pl-3 pr-2 text-left">
          {value.label}
        </div>
      </RadixPopover.Trigger>
      <RadixPopover.Content
        ...
        align="start"
        sideOffset={2}
        className="w-52 rounded-rounded border bg-grey-0 p-1"
      >
        {options.map((o, i) => (
          <div
            key={i}
            className="mb-1 flex rounded-rounded py-1.5 px-2 hover:bg-grey-5"
            onClick={() => onChange(o)}
          >
...
const CollapsibleWrapper = ({
  ...
  return (
    <div className={clsx("w-full border-b border-grey-5")}>
      ...
      >
        <RadixCollapsible.Trigger
          className={clsx(
            "flex w-full cursor-pointer items-center justify-between rounded py-1.5 px-3 text-grey-50"
          )}
        >
          <p>{title}</p>
