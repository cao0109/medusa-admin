...
const MetadataForm = ({ form }: MetadataProps) => {
  ...
  return (
    <>
      <div className="inter-base-regular divide-y divide-grey-20 rounded-rounded border border-grey-20">
        <div
          className={clsx(
            "inter-small-semibold rounded-t-rounded bg-grey-5",
            rowClasses
          )}
        >
          <div>
      ...
      {ignoredLength > 0 && (
        <div className="mt-base flex items-start gap-x-base rounded-rounded border border-[#FFD386] bg-[#FFECBC] p-base">
          <div>
            <WarningCircleIcon
              fillType="solid"
              size={20}
...
>(({ className, ...props }, ref) => {
  return (
    <input
      ref={ref}
      className={clsx(
        "placeholder:inter-base-regular w-full appearance-none outline-none placeholder:text-grey-40",
        className
      )}
      {...props}
    />
...
const MetadataRow = ({
  ...
  return (
    <div className="group/metadata relative last-of-type:rounded-b-rounded">
      <div className="grid grid-cols-[165px_1fr] divide-x divide-solid divide-grey-20 [&>div]:px-base [&>div]:py-xsmall">
        {children}
      </div>
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <Button
            ...
            size="small"
            className={clsx(
              "absolute inset-y-1/2 -right-small h-xlarge w-large -translate-y-1/2 transform opacity-0 transition-opacity group-hover/metadata:opacity-100 radix-state-open:opacity-100"
            )}
          >
        ...
        <DropdownMenu.Content asChild sideOffset={8}>
          <div className="z-50 overflow-hidden rounded-rounded border border-grey-20 bg-grey-0 shadow-dropdown">
            ...
            <DropdownMenu.DropdownMenuSeparator className="h-px w-full bg-grey-20" />
            <DropdownMenu.Item
              onClick={onDelete}
              disabled={isDisabled}
              className={clsx(
